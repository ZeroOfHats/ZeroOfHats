...
let generationCodeMirrorEditor; // Global reference for the Generation Script CodeMirror editor
...
async function refreshGameState() {
    try {
        ...
        if(currentScriptNameDisplay) currentScriptNameDisplay.textContent = state.current_script_name || 'default_room';
...
async function movePlayer(direction) {
    try {
        ...
        if (data.room_switched) {
            ...
            if (data.room_grid) currentRoomGrid = data.room_grid;
...
async function executeScript() {
    const scriptCode = generationCodeMirrorEditor.getValue(); // Get value from CodeMirror
    const scriptName = scriptNameInput.value || "untitled_live_run";
    updateGameLog([`Executing script: ${scriptName}`]);
    ...
    try {
        const response = await fetch('/generate_room_script', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ script_code: scriptCode, script_name: scriptName }),
        });
        const data = await response.json();
        ...
        entities = data.generated_entities || data.entities;
        playerId = data.player_id;
        if(currentScriptNameDisplay) currentScriptNameDisplay.textContent = scriptName;
        drawGrid();
...
async function saveScript() {
    ...
    try {
        const response = await fetch('/save_script', {
            ...
            body: JSON.stringify({ script_name: scriptName, script_code: scriptCode, script_type: GEN_SCRIPTS_SUBDIR}),
        });
        const data = await response.json();
...
async function loadSelectedScript() {
    ...
    try {
        ...
        if (data.script_code !== undefined) {
            generationCodeMirrorEditor.setValue(data.script_code); // Set value using CodeMirror
...
function copyScrapToEditor() {
    ...
    if (selectedScrapId && availableScraps[selectedScrapId]) {
        ...
        if (scrap.code) {
            ...
            generationCodeMirrorEditor.setCursor(generationCodeMirrorEditor.lineCount(), 0); // Move cursor to end
...
async function tickGame() {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        // const data = await response.json(); // Tick response might not need to return full state
        // For now, a full refresh is safest after a tick, as many things could change.
        await refreshGameState();
        // updateGameLog(data.game_log || [data.message || "Tick processed."]); // Log from refreshGameState will cover it
    } catch (error) {
