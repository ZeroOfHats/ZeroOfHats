...
let currentLabyrinthMap = null; // Holds the currently loaded LabyrinthMap object (as JS object)
let selectedRoomIdForEditing = null; // For room editor
let selectedRoomIdForConnection = null; // For connection editor source room
...
function lc_showAddRoomForm() {
    ...
    // roomGeneratorScriptSelect.value = ""; // Set to default or refresh
    roomParamsTextarea.value = "{}";
    deleteRoomButton.style.display = 'none';
    roomEditorArea.style.display = 'block';
    connectionEditorArea.style.display = 'none'; // Hide connection editor
...
function lc_editRoom(roomId) {
    ...
    roomIdInput.value = roomId; // Display current ID
    roomIdInput.disabled = true; // Don't allow changing ID of existing room
    roomDisplayNameInput.value = roomData.display_name || '';
    roomGeneratorScriptSelect.value = roomData.generator_script_name || '';
    roomParamsTextarea.value = JSON.stringify(roomData.room_params || {}, null, 2);
...
function lc_saveRoomDetails() {
    ...
    let roomParams;
    try {
        roomParams = JSON.parse(roomParamsTextarea.value || "{}");
    } catch (e) {
        alert("Room Params field contains invalid JSON.");
        return;
    }
    ...
    if (isEditing) {
        roomData.connections = currentLabyrinthMap.rooms[roomId].connections || {};
    } else {
    ...
    } else if (!currentLabyrinthMap.start_room_id) {
    ...
    updateMapDisplay();
    lc_hideRoomEditor();
    console.log(`Room '${roomId}' ${isEditing ? 'updated' : 'added'} to local map data.`);
}
...
// --- Existing Map Management JS (lc_refreshMapList, lc_createNewMap, lc_loadSelectedMap, lc_saveCurrentMap, lc_setActiveMap) ---
// These should be kept as they are from the previous subtask.
...
// Modify updateMapDisplay to handle room clicks
function updateMapDisplay() {
    ...
    // Hide connection editor if no room is selected for connection source
    if (!selectedRoomIdForConnection) { connectionEditorArea.style.display = 'none'; }
}
